...
import TreeImplementation.util.DrawUtils;
...
public class TreePainter {
    ...
    private static <T extends Comparable<T>> NodeDrawResult paint(Tree.SimpleTreeNode node, Graphics2D g2d,
                                                                  int x, int y) {
        if (node == null) {
            return null;
        }
        ...
        Color color = node.getGColor() == -1 ? Color.WHITE : new Color(255, node.getGColor(), 0);
        g2d.setColor(color);
    ...
    public static <T extends Comparable<T>> Point paint(Tree<T> tree, Graphics gr) {
